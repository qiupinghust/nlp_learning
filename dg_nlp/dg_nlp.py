...
trn_term_doc = vec.fit_transform(train_df[column])
test_term_doc = vec.transform(test_df[column])
y=(train_df["class"]-1).astype(int)
#这里可以调优
#lin_clf = svm.LinearSVC(C=0.1) #78.01%
#交叉验证
lin_clf = LogisticRegression(C=10,max_iter=1000) #78.40%
scores_count=cross_val_score(lin_clf,trn_term_doc,y,cv=10,scoring='accuracy')
